...
5. Large dialogs can resize within their parent window and expose a visible lower-right resize grip. Bands use draggable splitters with minimum widths; the content changes to a single scrolling column when three bands cannot fit. Header, footer, and primary action remain visible while content scrolls.

## Export dimensions and behavior

- Open near 1100 × 720 logical pixels, bounded by the available app window. Allow the outer frame to resize down to a compact layout and up to the host window margin.
...
- At less than 980 px dialog width, show output settings and styles in one scrollable column; hide the live preview. Never reduce controls below their useful width merely to keep all three bands visible.
